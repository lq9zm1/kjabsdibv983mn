#!/usr/bin/env python3
"""ETF top-15 holdings via Alpha Vantage ETF_PROFILE -> BigQuery etf_top_holdings (WRITE_TRUNCATE)."""
...
def fetch(etf):
    ...
    for h in hold:
        ...
        try: w = float(h.get("weight") or 0)
        except (TypeError, ValueError): w = 0.0
        if not tk or tk in ("N/A", ""): continue
...
def main():
    ...
    for i, etf in enumerate(ETFS):
        try: top, err = fetch(etf)
        except Exception as e:
            print(f"  {etf:5s} FETCH FAILED: {e}", flush=True); continue
        if not top:
            print(f"  {etf:5s} no holdings -> {str(err)[:180]}", flush=True); continue
        ...
        if i < len(ETFS) - 1: time.sleep(13)     # free tier ~5 req/min
    if not rows:
        sys.exit("ERROR: nothing parsed — check key / daily limit (messages above)")
    schema = [bigquery.SchemaField("etf","STRING"), bigquery.SchemaField("rank","INT64"),
              bigquery.SchemaField("ticker","STRING"), bigquery.SchemaField("weight","FLOAT64"),
              bigquery.SchemaField("name","STRING"), bigquery.SchemaField("as_of","DATE")]
