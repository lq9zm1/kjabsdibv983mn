...
final AS (
  SELECT
    ...
    CASE WHEN r.ticker='SPY' THEN 50 ELSE ROUND(rk.cd_1d3d,0)  END AS rs_1d_3d,
    CASE WHEN r.ticker='SPY' THEN 50 ELSE ROUND(rk.cd_3d7d,0)  END AS rs_3d_7d,
    CASE WHEN r.ticker='SPY' THEN 50 ELSE ROUND(rk.cd_7d14d,0) END AS rs_7d_14d,
  ...
  CROSS JOIN stats st
  CROSS JOIN sd
  LEFT JOIN ranked rk USING (ticker)
)
