-- ============================================================================
-- leader_tier_backfill — ONE-TIME FULL-HISTORY build of the leader/tier backend (1962+).
--   NOT a nightly file — do NOT put this in sql/. The nightly (sql/16_leader_tier_nightly.sql) only
--   upserts the recent days; THIS rebuilds all of history and is expensive.
--   RUN IT: (1) once to seed everything, and (2) whenever the theme map changes (adds/removes/moves shift
--   the historical bands). The ticker_review executor (apply_review_actions.py) re-runs this automatically
--   right after it rebuilds stock_theme_map, so a submit that alters themes refreshes the full history in
--   that same nightly — normal nights never touch history.
--
-- Builds: (A) v_leader_status_daily (universe/etf flags) · (B) leader_tier_daily (Leader/Mid/Laggard, full
--   history) · (C) leader_tier_spells (transitions) · (D) v_leader_tier_current · (E) v_leader_tier_primary.
-- group_score = AVG(ret_1d,1w,1m,3m,6m) from bt_leader_daily (matches theme_stats + the fixed v_stock_dashboard).
--
-- ⚠ BACKEND SYNC: also apply the v_stock_dashboard group_score fix (AVG, no atr_pct) so the live Theme
--   View and this historical tier stay identical.
-- ============================================================================
